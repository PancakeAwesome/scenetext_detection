...
path = '/Volumes/Macintosh\ HD/Documents/study/AI/Character_Recognition/ctpn_tensorflow_notes/data/mlt_english+chinese/image'
gt_path = '/Volumes/Macintosh\ HD/Documents/study/AI/Character_Recognition/ctpn_tensorflow_notes/data/mlt_english+chinese/label'
out_path = 're_image'
...
for file in files:
    ...
    with open(gt_file, 'r') as f:
        lines = f.readlines()
    for line in lines:
        # 一般数据库给的都是整个文本行或者单词级别的标注。
        # 因此需要把这些标注转换成一系列固定宽度的ground text proposals
        splitted_line = line.strip().lower().split(',')
        pt_x = np.zeros((4, 1))
        pt_y = np.zeros((4, 1))
        # ground truth的格式是：[x1,y1,x2,y2,x3,y3,x4,y4]
        # 得到文本行标注在缩放后图片上的坐标
        ...
        # ground truth转换成固定宽度为16像素的左上坐标和右下坐标的text proposals
